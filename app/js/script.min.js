        ...
        tels.forEach((tel)=>{
           // https://intl-tel-input.com/
            ...
            function handleMaskChange(isForward, key){
                ...
                if (Array.isArray(masks[countrycode])){
                    if (isForward == 0){
                        ...
                        let inputLength = phoneInput.val().match(/\d+/g).join('').length
                ...
                if (key.length) phoneInput.val(phoneInput.val().replace('_', key))
            }
            ...
            tel.addEventListener('keydown', function(e){
                ...
                let maskIdx = phoneInput.data('maskidx')
                let key = event.key
                if (/^[0-9]$/i.test(key) && /^[0-9]$/i.test(phoneInput.val()[phoneInput.val().length - 1]) && inputLength + 1 > mask && masks[countrycode].length > maskIdx + 1){
                    handleMaskChange(1, key)
                }
                else if ((key === "Backspace" || key === "Delete") && mask && maskIdx > 0 && masks[countrycode][maskIdx - 1].match(/#+|\d+/g).join('').length >= phoneInput.val().match(/\d+/g).join('').slice(0, -1).length){
                    handleMaskChange(-1, '')
                }
